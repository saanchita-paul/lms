...
class MonthlyReports extends Command
{
     ...
     * @var string
     */
    protected $signature = 'mailcampaign:monthly';
     ...
     * @var string
     */
    protected $description = 'Respectively send an exclusive reports to everyone monthly via email.';
    ...
    public function handle()
    {
        $clinics = Clinic::where('status','Active')->where('emailcampaign',"1")->get();  
         ...
         foreach ($clinics as $key => $value) {
                ...
                $job = (new MonthlyMail($value))->onQueue('monthlymailcampaign');
