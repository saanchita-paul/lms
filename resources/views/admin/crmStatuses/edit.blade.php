...
<div class="card-content">
    <div class="card-header">
        <h5>{{ trans('global.edit') }} {{ trans('cruds.crmStatus.title_singular') }}</h5>
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.crm-statuses.update", [$crmStatus->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('board') ? 'is-invalid' : '' }}" type="text" name="board" id="board" value="{{ old('board', $crmStatus->board) }}" required>
                @if($errors->has('board'))
                    <div class="invalid-feedback">
                        {{ $errors->first('board') }}
                    </div>
