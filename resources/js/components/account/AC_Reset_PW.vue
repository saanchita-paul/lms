<template>
    <div class="welcome-box p-5">
        <div class="welcome-box-info">
            <h1 class="d-inline-block align-top">Reset password</h1>
            ...
            <p>Enter following details to reset your password.</p>
            <form action="#" @submit.prevent="submit">
                <div class="errors mt-2 px-5" v-if="authStore.resetPWMessage!=''">
                    <div class="alert" :class="authStore.resetPWError ? 'alert-danger' : 'alert-success'" role="alert">
                        {{ authStore.resetPWMessage }}
                    </div>
                </div>
                <div class="row gx-3">
                    <div class="col-md-10 col-lg-8 offset-md-1 offset-lg-2">
                        <div class="form-group">
                            <input type="email" class="form-control" :class="{'filled':form.email}" placeholder="" v-model="form.email">
                            <label>Email Address</label>
                        </div><!--/.form-group-->
                        <div v-if="errors['email']" class="alert alert-danger" role="alert" style="font-size:14px; padding:5px;">
                            {{ errors['email'] }}
                        </div>
                    </div>
                </div>
                <div class="row gx-3">
                    <div class="col-md-10 col-lg-8 offset-md-1 offset-lg-2">
                        <div class="form-group">
                            <input type="password" class="form-control" :class="{'filled':form.password}" placeholder="" v-model="form.password">
                            <label>Password</label>
                        </div><!--/.form-group-->
                        <div v-if="errors['password']" class="alert alert-danger" role="alert" style="font-size:14px; padding:5px;">
                            {{ errors['password'] }}
                        </div>
                    </div>
                </div>
                <div class="row gx-3">
                    <div class="col-md-10 col-lg-8 offset-md-1 offset-lg-2">
                        <div class="form-group">
                            <input type="password" class="form-control" :class="{'filled':form.password_confirmation}" placeholder="" v-model="form.password_confirmation">
                            <label>Password Confirmation</label>
                        </div><!--/.form-group-->
                        <div v-if="errors['password_confirmation']" class="alert alert-danger" role="alert" style="font-size:14px; padding:5px;">
...
export default {
    ...
    methods: {
        submit(){
            ...
            this.authStore.resetPWMessage = '';
            this.authStore.resetPWError = false;
            ...
            if(!this.form.password){
                ...
                this.errors['password'] = 'Password field must be of 8 characters.';
            }

            if(!this.form.password_confirmation){
                this.errors['password_confirmation'] = 'Password Confirmation field is required.';
            }else if(this.form.password !== this.form.password_confirmation ){
                this.errors['password_confirmation'] = 'Password Confirmation field is not same as password field.';
            }

            this.authStore.resetpw(this.form)
        },
        signin(){
