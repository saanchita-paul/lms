...
export const useLeadStore = defineStore({
    ...
    actions: {
        ...
        async add() {
            ...
            this.lead.clinic_id = this.authStore.clinic_id;

            return axios.post('/api/v1/lead/store', this.lead, config)
                .then(function (response) {
                    //console.log(response);
                    if (response.data.success) {
                        ...
                        _self.errorMessage = response.data.message;
        ...
        async move(status_id, leadid, reason = null) {
            let _self = this;
            const config = this.getConfig();

            return axios.post('/api/v1/lead/movestage', {status_id: status_id, reason: reason, leadid: leadid}, config)
                .then(function (response) {
                    //console.log(response);
                    if (response.data.success) {
                        _self.alertStore.success = true;
        ...
        searchLeads(searchTerm) {
            ...
            axios.get('/api/v1/search', {params: {search: searchTerm, clinic_id: this.authStore.clinic_id}}, config)
                .then(function (response) {
                    if (response.data.success) {
                        _self.searchedLeads = response.data.data.data;
                    } else {
                        _self.searchedLeads = [];
                    }
                })
            ...
            axios
                ...
                .then(function (response) {
                    if (response.data.success) {
                        ...
                        _self.alertStore.success = true;
                        _self.alertStore.message = 'Lead Updated Successfully!';
                        _self.list();
                        _self.count();
